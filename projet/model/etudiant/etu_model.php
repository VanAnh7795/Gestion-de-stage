<?php
include_once ("../../public/libraries/db_config.php");
function sou_liste(){
	$db = connect_db();
	$SQL = "SELECT  etu.eid, etu.nom as etu_nom, etu.prenom as etu_prenom, 
	sou.date, sou.salle,
...
function ajout_tab_etu($nom, $prenom, $email, $tel){
	$db = connect_db();
	$req = $db->prepare('INSERT INTO etudiants (nom, prenom, email, tel) 
...
function get_last_etu(){
	...
	WHERE eid = (SELECT MAX(eid) FROM etudiants)";
	$stmt = $db-> query($SQL);
	$res = $stmt->fetchAll();
	return $res[0];
}
...
function info_reference_dossier($eid){
	$db = connect_db();
	$SQL = "SELECT titre, tuteurP, date, tuteur1, tuteur2, salle, note, commentaire 
	FROM etudiants etu
	LEFT JOIN  stages ON etu.eid = stages.eid 
	LEFT JOIN soutenances sou ON stages.sid=sou.sid 
